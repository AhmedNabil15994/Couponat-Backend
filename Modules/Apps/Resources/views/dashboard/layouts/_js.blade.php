...
<script>
    ...
    {
        $('#seller_id').select2({
            placeholder: "{{ __('apps::dashboard._layout.aside.sellers') }}",
        })
...
<script>
  ...
  $(document).ready(function()
  {
      ...
      $('select[name="search_offer"]').select2({
          placeholder: 'Search By Offer Code',
          ajax: {
              url: "{{route('dashboard.offers.searchAjax')}}",
              dataType: 'json',
                      ...
                      results: $.map(data, function (item) {
                          if(item.id){
                              return {
                                  ...
                                  text: item.code+' '+item.offer.title.{{locale()}},
                                  id: item.code,
                              }
                          }
                      })
      ...
      $('select[name="search_offer"]').on('change',function(){
          // let orderId = $(this).children('option:selected').data().data.source;
          {{--let baseUrl =  "{{route('dashboard.orders.show',":id")}}".replace(':id',orderId);--}}
          // window.location.href= baseUrl + "?item_code="+$(this).val()
      });
