...
class ReportRepository
{
    ...
    public function QueryVendors($request)
    {
        ...
        if(isset($request['req']['seller_id']) && !empty($request['req']['seller_id'])){
            $query = $query->where('id',$request['req']['seller_id']);
        }
        ...
        $query = $query->Sellers()->SellersActiveOrders()
    ...
    public function QueryCustomers($request)
    {
        ...
        $query = $query->doesnthave('roles')
            ...
            ->withSum(['orders'=>function ($query) use ($request) {
                if (isset($request['req']['from']) && $request['req']['from'] != '') {
                    $query->whereDate('created_at', '>=', $request['req']['from']);
                }
                if (isset($request['req']['to']) && $request['req']['to'] != '') {
    ...
    public function QueryOffers($request)
    {
        ...
        if (isset($request['search']['value']) && !empty($request['search']['value'])) {
            $query = $query->where(DB::raw('lower(title)'),'LIKE','%'.strtolower($request['search']['value']).'%')
                ->orWhere(DB::raw('lower(discount_desc)'),'LIKE','%'.strtolower($request['search']['value']).'%');
        }

        $query = $query->withSum(['orderItems'=>function ($q) use ($request) {
                $q->whereHas('order.orderStatus',fn($q) => $q->successPayment());
                if (isset($request['req']['from']) && $request['req']['from'] != '') {
                    $q->whereDate('created_at', '>=', $request['req']['from']);
                }
                if (isset($request['req']['to']) && $request['req']['to'] != '') {
            ...
            ->withCount(['orderItems'=> function ($q) use ($request) {
                $q->whereHas('order.orderStatus',fn($q) => $q->successPayment());
                if (isset($request['req']['from']) && $request['req']['from'] != '') {
                    $q->whereDate('created_at', '>=', $request['req']['from']);
                }
